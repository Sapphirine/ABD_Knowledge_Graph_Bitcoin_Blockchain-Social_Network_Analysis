...
# db_path: the string path to write the database files
# transactions_in: the string path to find the raw transaction file
# transaction_keys: the string path to write the transaction strings
# pub_keys: the string path to write the pubkey strings
#
# This script preprocesses the raw bitcoin text data, numerically indexing the lengthy string data and outputting a long int matrix.
#
##
...
def write_network_dictionaries(db_path, transactions_in, transaction_keys, pub_keys):
    ...
    for row in f: #each file line
        ...
        if add == 1:
            date = int(date[0:4] + date[5:7] + date[8:10] + date[11:13] + date[14:16] + date[17:19]) 
