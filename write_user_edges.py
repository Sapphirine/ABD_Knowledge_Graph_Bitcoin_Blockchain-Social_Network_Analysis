...
#   referents_out: the string path of where to output the transaction_id referents
#   public_keys_out: the string path of where to output the pubkey_id referents
#   edges_out: the string path of where to output the main data file (edges)
#   user_hash: the dictionary mapping pubkeys to user_ids (generated in write_user_public_keys.py)
#   return None
#
#   This script outputs the files for referent transaction information and main data edges.
# 
#
...
def write_user_edges(referents_out, public_keys_out, edges_out, rows, user_hash):
    ...
    for row in rows:
        ...
        if row[0] == 1: #if output
            ...
            line = [int(row[1]), int(user_hash[pubkey]), int(user_hash[row[3]]), int(row[4]), str(round(float(row[5])*1.0e-9, 8))] 
